...
<div class="bg-slate-800 rounded-xl shadow-lg p-6 border border-slate-700">
    ...
    <div class="mb-4 grid grid-cols-1 md:grid-cols-3 gap-4">
        <div class="relative">
            <i class="fas fa-search text-slate-500 absolute top-3.5 left-3"></i>
            <input type="text" id="searchInput" placeholder="Cari barang..." class="bg-slate-900 border border-slate-600 text-slate-200 pl-10 pr-4 py-2 w-full rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500">
        </div>
    ...
    <div class="overflow-x-auto">
        <table class="min-w-full text-sm" id="barangTable">
            ...
            <tbody class="text-slate-400">
                @forelse($barang as $item)
                    <tr class="border-b border-slate-700 hover:bg-slate-700/50">
                        <td class="px-4 py-2 text-center">
                            <img src="{{ $item->foto ? asset('storage/barang/' . $item->foto) : 'https://placehold.co/100x100/1e293b/475569?text=N/A' }}" alt="{{ $item->nama_barang }}" class="w-12 h-12 object-cover rounded-md inline-block">
                        </td>
                        ...
                        <td class="px-4 py-3 text-center">
                            <div class="flex items-center justify-center space-x-3">
                                ...
                                <form action="{{ route('barang.destroy', $item->id_barang) }}" method="POST" onsubmit="return confirm('Anda yakin ingin menghapus barang ini?')" class="inline">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="text-red-500 hover:text-red-400" title="Hapus"><i class="fas fa-trash"></i></button>
                                </form>
                            </div>
                        </td>
                    </tr>
                @empty
                    <tr>
                        <td colspan="9" class="text-center py-10 text-slate-500">Tidak ada data barang ditemukan.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
